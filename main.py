...
class DiceGame:
    def __init__(self, root):
        ...
        self.dice_images = [ImageTk.PhotoImage(Image.open(f'dice_png/{i}.png')) for i in
                            range(1, 7)]
        ...
        self.dice1_label.grid(row=0, column=0)

        self.dice2_label = tk.Label(root)
        self.dice2_label.grid(row=0, column=1)

        self.roll_button = tk.Button(root, text="Roll!", command=self.roll_dice)
        self.roll_button.grid(row=1, column=0, columnspan=2)
